...
export default function POSPage() {
  ...
  function addToCart(product: Product, qty = 1) {
    ...
    setCart((prev) => {
      ...
      return [...prev, { ...product, qty, unitPrice: product.sellingPrice }]
    })
                ...
                <div key={item.id} className="rounded-xl border border-gray-200 bg-white min-w-0 overflow-hidden">
                  <div className="flex items-start justify-between gap-2 px-3 pt-3 pb-2">
                    ...
                    <button
                      ...
                      className={`rounded-md text-gray-400 shrink-0 ${
                        touchMode
                          ? 'min-h-9 min-w-9 flex items-center justify-center active:bg-red-50 active:text-red-600 active:scale-95'
                  ...
                  <div className="mx-3 mb-3 rounded-lg border border-amber-100 bg-amber-50/70 p-2.5 space-y-2">
                    <div className="flex items-center justify-between gap-2 text-[11px] tabular-nums">
                      <span className="text-gray-500">
                        List{' '}
                        <span className={displayUnitPrice !== item.sellingPrice ? 'line-through' : 'font-medium text-gray-700'}>
                          KES {item.sellingPrice.toLocaleString()}
                        </span>
                      </span>
                      {discountable && (
                        <span className="text-amber-800/90 shrink-0">
                    ...
                    <div className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-2">
                      ...
                      <button
                        ...
                        className={`rounded-md px-2.5 py-1.5 text-[11px] font-medium transition-colors disabled:opacity-40 ${
                          priceSavePending
                  ...
                  <div className="border-t border-gray-100 bg-gray-50/60 px-3 py-3 space-y-2.5">
                    <CartQtyControl
                      ...
                      stockAvailable={stockByProductId[item.id] ?? item.initialStock ?? 0}
