...
import { getByCategory } from '@/lib/db/products'
...
export default function CategoryModal({ category, onClose }: Props) {
  ...
  useEffect(() => {
    getByCategory(category.id).then(setProducts)
  }, [category.id])
    ...
    >
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5">
          {products.length === 0 ? (
            <p className="text-sm text-gray-500 text-center py-16">No products in this category</p>
          ) : (
